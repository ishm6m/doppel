...
- [~] **T8.2** Performance harness + budgets (`PERFORMANCE.md`). **Infra done:** `Tools/CorpusGen` generates deterministic synthetic corpora (seeded, known unique/exact/near ratios) via `swift run --package-path Tools/CorpusGen CorpusGen --out … --count …`; `PerfHarnessTests` runs the full cascade over a generated corpus, asserts a regression ceiling, and appends throughput to a CSV. Local debug baseline: **600 docs ≈ 390 files/sec** (already > the 300 files/sec release target). **Remaining:** the headline budget assertion (50k docs < 5 min, < 1.5 GB peak) is release-mode on Apple Silicon — a target-hardware / nightly run (and needs the CI runner unblocked); peak-memory metric not yet wired.
